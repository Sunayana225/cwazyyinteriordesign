      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {galleryItems.map(({ label, palette, desc }, i) => (
          ...
          >
            ...
            <div className="p-5">
              <h3 className="font-serif text-lg text-charcoal-600 mb-1">{label}</h3>
      ...
      <div className="grid md:grid-cols-3 gap-10 md:gap-16">
        {features.map(({ number, title, description }, i) => (
          <motion.div
            ...
            variants={fadeUp}
          >
            <p className="font-serif text-4xl text-taupe-200 mb-4 leading-none">{number}</p>
            <h3 className="font-serif text-xl text-charcoal-600 mb-3">{title}</h3>
            <p className="text-charcoal-400 leading-relaxed text-sm">{description}</p>
          </motion.div>
        ))}
      </div>
    </div>
