...
function getUserEmailFromRequest(req: Request): string | undefined {
  return req.user?.email ?? undefined;
}
...
async function writeAudit(opts: { actorEmail?: string; action: string; designId?: string; targetId?: string; before?: unknown; after?: unknown; }): Promise<void> {
  try {
    await pool.query(
      `INSERT INTO alveo_design_audit (actor_email, action, design_id, target_id, before_data, after_data)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [opts.actorEmail ?? null, opts.action, opts.designId ?? null, opts.targetId ?? null, opts.before !== undefined ? JSON.stringify(opts.before) : null, opts.after !== undefined ? JSON.stringify(opts.after) : null],
    );
  } catch {
    return;
  }
...
async function getPermissions(designId: string): Promise<DesignCommentPermissions> {
  ...
  return { ownerEmail: row.owner_email ?? undefined, defaultRole: (row.default_role as "viewer" | "editor") ?? "editor", editors: row.editors ?? [] };
}
...
const postSchema = z.object({ designId: z.string().min(1).max(200), text: z.string().min(1).max(500), parentId: z.string().min(1).max(200).optional() });
const patchPermissionSchema = z.object({ designId: z.string().min(1).max(200), defaultRole: z.enum(["viewer", "editor"]).optional(), addEditor: z.string().email().optional(), removeEditor: z.string().email().optional(), transferOwner: z.string().email().optional() });
const mentionAckSchema = z.object({ action: z.literal("mention-ack"), mentionUser: z.string().min(1).max(120), commentId: z.string().min(1).max(200), read: z.boolean() });

interface CommentRow { id: string; design_id: string; user_email: string; author_name: string; text: string; parent_id: string | null; mentions: string[]; mention_read: boolean; created_at: string; }

function rowToComment(r: CommentRow) {
  return { id: r.id, designId: r.design_id, text: r.text, author: r.author_name, createdAt: r.created_at, parentId: r.parent_id ?? undefined, mentions: r.mentions ?? [], mentionRead: r.mention_read };
}
...
router.get("/design-comments", async (req: Request, res: Response) => {
  ...
  if (isAll) {
    ...
    try {
      ...
      const rows = await pool.query<CommentRow>(`SELECT * FROM alveo_design_comments ${where} ORDER BY ${orderCol} ${orderBy} LIMIT $${pi++} OFFSET $${pi++}`, [...params, pageSize, offset]);
  ...
  try {
    ...
    if (mentionKeys.length > 0) {
      const readRows = await pool.query<{ comment_id: string }>(`SELECT comment_id FROM alveo_mention_reads WHERE mention_user = ANY($1) AND read = TRUE`, [mentionKeys]);
      for (const r of readRows.rows) relevantRead.add(r.comment_id);
    }
    const rows = await pool.query<CommentRow>(`SELECT * FROM alveo_design_comments WHERE design_id = $1 ORDER BY created_at ASC`, [designId]);
    const comments = rows.rows.map((r) => { const base = rowToComment(r); const hasMen = mentionKeys.some((k) => (r.mentions ?? []).includes(k)); return { ...base, mentionRead: !hasMen || relevantRead.has(r.id) }; });
    res.json({ comments, permissions, role, canManage: canManagePermissions(permissions, userEmail), unreadMentionCount: comments.filter((c) => !c.mentionRead).length });
  } catch {
    res.status(500).json({ error: "Database error" });
  }
});
...
router.post("/design-comments", requireAuth, async (req: Request, res: Response) => {
  ...
  if (!limit.allowed) { res.setHeader("Retry-After", String(limit.retryAfterSec)); res.status(429).json({ error: "Too many requests" }); return; }
  const parsed = postSchema.safeParse(req.body);
  if (!parsed.success) { res.status(400).json({ error: "Invalid request body" }); return; }
  try {
    let permissions = await getPermissions(parsed.data.designId);
    if (!permissions.ownerEmail) { permissions = { ownerEmail: userEmail, defaultRole: "editor", editors: [] }; await upsertPermissions(parsed.data.designId, permissions); }
    if (getEffectiveRole(permissions, userEmail) !== "editor") { res.status(403).json({ error: "Read-only comments" }); return; }
    const id = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const mentions = extractMentions(parsed.data.text);
    await pool.query(`INSERT INTO alveo_design_comments (id, design_id, user_email, author_name, text, parent_id, mentions) VALUES ($1, $2, $3, $4, $5, $6, $7)`, [id, parsed.data.designId, userEmail, userEmail, parsed.data.text.trim(), parsed.data.parentId ?? null, mentions]);
    await writeAudit({ actorEmail: userEmail, action: "comment.create", designId: parsed.data.designId, targetId: id, after: { text: parsed.data.text.trim(), parentId: parsed.data.parentId ?? null, mentions } });
    const rows = await pool.query<CommentRow>(`SELECT * FROM alveo_design_comments WHERE design_id = $1 ORDER BY created_at ASC`, [parsed.data.designId]);
...
router.patch("/design-comments", requireAuth, async (req: Request, res: Response) => {
  const configuredToken = process.env["EVENTS_ADMIN_TOKEN"];
  const userEmail = getUserEmailFromRequest(req);
  const mentionAckParsed = mentionAckSchema.safeParse(req.body);
  if (mentionAckParsed.success) {
    if (!configuredToken || req.headers["x-admin-token"] !== configuredToken) { res.status(401).json({ error: "Unauthorized" }); return; }
    const { mentionUser, commentId, read } = mentionAckParsed.data;
    const key = mentionUser.trim().toLowerCase();
    try {
      if (read) {
        await pool.query(`INSERT INTO alveo_mention_reads (mention_user, comment_id, read) VALUES ($1, $2, TRUE) ON CONFLICT (mention_user, comment_id) DO UPDATE SET read = TRUE`, [key, commentId]);
      } else {
        await pool.query(`INSERT INTO alveo_mention_reads (mention_user, comment_id, read) VALUES ($1, $2, FALSE) ON CONFLICT (mention_user, comment_id) DO UPDATE SET read = FALSE`, [key, commentId]);
      }
      await writeAudit({ actorEmail: userEmail, action: "mention.ack", targetId: commentId, after: { mentionUser: key, read } });
      const countResult = await pool.query<{ count: string }>(`SELECT COUNT(*) FROM alveo_mention_reads WHERE mention_user = $1 AND read = TRUE`, [key]);
  ...
  try {
    ...
    if (!current.ownerEmail) { current = { ownerEmail: userEmail, defaultRole: "editor", editors: [] }; }
    if (!canManagePermissions(current, userEmail)) { res.status(403).json({ error: "Forbidden" }); return; }
    const actions: string[] = [];
    if (parsed.data.defaultRole) { actions.push(`defaultRole:${current.defaultRole}→${parsed.data.defaultRole}`); current.defaultRole = parsed.data.defaultRole; }
    if (parsed.data.addEditor) { const n = parsed.data.addEditor.toLowerCase(); if (!current.editors.map((v) => v.toLowerCase()).includes(n)) { current.editors.push(parsed.data.addEditor); actions.push(`addEditor:${parsed.data.addEditor}`); } }
    if (parsed.data.removeEditor) { current.editors = current.editors.filter((v) => v.toLowerCase() !== parsed.data.removeEditor!.toLowerCase()); actions.push(`removeEditor:${parsed.data.removeEditor}`); }
    if (parsed.data.transferOwner) { current.ownerEmail = parsed.data.transferOwner; current.editors = current.editors.filter((v) => v.toLowerCase() !== parsed.data.transferOwner!.toLowerCase()); actions.push(`transferOwner:${parsed.data.transferOwner}`); }
    await upsertPermissions(parsed.data.designId, current);
    await writeAudit({ actorEmail: userEmail, action: "permissions.update", designId: parsed.data.designId, before, after: { ...current, changes: actions } });
