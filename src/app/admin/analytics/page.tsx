...
export default function AnalyticsAdminPage() {
  ...
  async function loadEvents() {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 401) {
          setError("Unauthorized. Provide a valid admin token.");
        } else {
  ...
  async function setMentionRead(commentId: string, read: boolean) {
    ...
    setMentionQueue((current) =>
      current.map((comment) =>
        comment.id === commentId ? { ...comment, mentionRead: read } : comment,
      ),
    );
  ...
  useEffect(() => {
    ...
    // We intentionally refetch when token changes.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [token]);

  useEffect(() => {
    void loadMentionQueue();
    // intentionally tied to mention filter updates
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const funnel = useMemo(
    () => buildFunnel(filteredEvents, FUNNEL_STEPS),
    [filteredEvents],
  );

  const dailyTrend = useMemo(
    () => buildDailyStepTrend(filteredEvents, FUNNEL_STEPS, 7),
    [filteredEvents],
  );

  const sessionJourneys = useMemo(
    () => buildSessionJourneys(filteredEvents, FUNNEL_STEPS),
    [filteredEvents],
  );
  ...
  const funnelVisual = useMemo(() => {
    ...
    return funnel.map((step) => {
      const relativePct = first > 0 ? Math.max(6, Math.round((step.count / first) * 100)) : 0;
      return {
        ...step,
        relativePct,
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="font-serif text-4xl text-charcoal-600 dark:text-cream-100 mt-1">
              Analytics Dashboard
            </h1>
            <p className="text-sm text-charcoal-400 dark:text-cream-200 mt-2 max-w-2xl">
              Live operational feed for product usage and event funnel health.
            </p>
          </div>
          <button
        ...
        <section className="grid gap-4 md:grid-cols-3">
          <div className="rounded-2xl bg-white dark:bg-charcoal-500 border border-cream-200 dark:border-charcoal-400 p-5">
            <p className="text-xs uppercase tracking-wider text-taupe-400">Events Cached</p>
            <p className="mt-2 text-3xl font-serif text-charcoal-600 dark:text-cream-100">{filteredEvents.length}</p>
          </div>
          <div className="rounded-2xl bg-white dark:bg-charcoal-500 border border-cream-200 dark:border-charcoal-400 p-5">
            <p className="text-xs uppercase tracking-wider text-taupe-400">Past 24h</p>
            <p className="mt-2 text-3xl font-serif text-charcoal-600 dark:text-cream-100">{last24h}</p>
          </div>
          <div className="rounded-2xl bg-white dark:bg-charcoal-500 border border-cream-200 dark:border-charcoal-400 p-5">
            <p className="text-xs uppercase tracking-wider text-taupe-400">Unique Event Types</p>
            <p className="mt-2 text-3xl font-serif text-charcoal-600 dark:text-cream-100">{counts.length}</p>
          </div>
        </section>
        ...
        <section className="rounded-2xl bg-white dark:bg-charcoal-500 border border-cream-200 dark:border-charcoal-400 p-5">
          <div className="grid gap-4 md:grid-cols-3">
            <div>
              <label className="block text-xs uppercase tracking-wider text-taupe-400" htmlFor="admin-token">
              ...
              <input
                ...
                placeholder="Paste EVENTS_ADMIN_TOKEN"
                className="mt-2 w-full rounded-lg border border-cream-200 dark:border-charcoal-400 bg-white dark:bg-charcoal-600 px-3 py-2.5 text-sm text-charcoal-600 dark:text-cream-100"
              />
            </div>

            <div>
              <label className="block text-xs uppercase tracking-wider text-taupe-400" htmlFor="date-range">
                Date Range
              </label>
              <select
                id="date-range"
            ...
            <div>
              <label className="block text-xs uppercase tracking-wider text-taupe-400" htmlFor="event-filter">
                Event Filter
              </label>
              <input
                id="event-filter"
          ...
          <p className="mt-2 text-xs text-charcoal-400 dark:text-cream-300">
            Token is stored only in this browser via localStorage.
          </p>
        ...
        <section className="rounded-2xl bg-white dark:bg-charcoal-500 border border-cream-200 dark:border-charcoal-400 p-5">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-xs text-charcoal-400 dark:text-cream-300 mt-1">
                Recent comments that mention a target user.
              </p>
            </div>
            <div className="grid gap-2 md:grid-cols-3 w-full md:w-auto">
              <input
                ...
                placeholder="mention username (without @)"
                className="rounded-lg border border-cream-200 dark:border-charcoal-400 bg-white dark:bg-charcoal-600 px-3 py-2 text-sm"
              />
              <input
                type="date"
                value={mentionFromDate}
                onChange={(e) => setMentionFromDate(e.target.value)}
                className="rounded-lg border border-cream-200 dark:border-charcoal-400 bg-white dark:bg-charcoal-600 px-3 py-2 text-sm"
              />
              <input
                type="date"
                value={mentionToDate}
                onChange={(e) => setMentionToDate(e.target.value)}
                className="rounded-lg border border-cream-200 dark:border-charcoal-400 bg-white dark:bg-charcoal-600 px-3 py-2 text-sm"
              />
            </div>
          </div>
          ...
          {mentionQueue.length === 0 ? (
            ...
            <ul className="mt-4 space-y-2 max-h-[260px] overflow-auto pr-1">
              {mentionQueue.slice(0, 20).map((comment) => (
                <li
                  key={comment.id}
                  className="rounded-lg border border-cream-100 dark:border-charcoal-400 p-3"
                >
                  <p className="text-sm text-charcoal-600 dark:text-cream-100">{comment.text}</p>
                  <p className="text-xs text-charcoal-400 dark:text-cream-300 mt-1">
                    {comment.author} · design {comment.designId} · {formatTime(comment.createdAt)}
                  </p>
                  <div className="mt-2 flex items-center justify-end gap-2">
                    ...
                    <button
                      onClick={() => void setMentionRead(comment.id, !comment.mentionRead)}
        ...
        <section className="rounded-2xl bg-white dark:bg-charcoal-500 border border-cream-200 dark:border-charcoal-400 p-5">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-4">
            {funnel.map((item) => (
              <div
                key={item.step}
                className="rounded-xl border border-cream-100 dark:border-charcoal-400 p-3"
              >
                ...
                <p className="text-xs text-charcoal-400 dark:text-cream-300 mt-1">
                  {item.conversionFromPrevPct}% from previous step
                </p>
              </div>
            ))}
          </div>
            ...
            {funnelVisual.map((item) => (
              <div key={`bar-${item.step}`}>
                <div className="flex items-center justify-between text-xs mb-1">
                  <span className="uppercase tracking-wider text-charcoal-400 dark:text-cream-300">
                    {item.step}
                  </span>
                  <span className="text-charcoal-400 dark:text-cream-300">
                    Drop-off: {item.dropOffPct}%
                  </span>
                </div>
                <div className="h-2.5 rounded-full bg-cream-100 dark:bg-charcoal-600 overflow-hidden">
                  <div
                    className="h-full bg-charcoal-600 dark:bg-cream-100 transition-all"
            ...
            {loading ? (
              ...
              <ul className="mt-4 space-y-2">
                {counts.slice(0, 12).map((item) => (
                  <li
                    key={item.name}
                    className="flex items-center justify-between text-sm border-b border-cream-100 dark:border-charcoal-400 pb-2"
                  >
              ...
              <ul className="mt-4 space-y-3 max-h-[420px] overflow-auto pr-1">
                {filteredEvents
                  ...
                  .map((event, index) => (
                    ...
                    >
                      <p className="text-sm font-medium text-charcoal-600 dark:text-cream-100">
                        {event.name}
                      </p>
                      <p className="text-xs text-charcoal-400 dark:text-cream-300 mt-1">
                        {formatTime(event.at)}
        ...
        <section className="grid gap-6 lg:grid-cols-2">
                ...
                <thead>
                  <tr className="text-left text-taupe-400 uppercase tracking-wider">
                    ...
                    {FUNNEL_STEPS.map((step) => (
                      <th key={step} className="pb-2 pr-3">{step}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {dailyTrend.map((row) => (
                    <tr key={row.day} className="border-t border-cream-100 dark:border-charcoal-400">
                      ...
                      {FUNNEL_STEPS.map((step) => (
                        <td key={`${row.day}-${step}`} className="py-2 pr-3 text-charcoal-400 dark:text-cream-300">
                          {row.counts[step]}
                        </td>
                      ))}
                    </tr>
          ...
          <div className="rounded-2xl bg-white dark:bg-charcoal-500 border border-cream-200 dark:border-charcoal-400 p-5">
            ...
            <p className="text-xs text-charcoal-400 dark:text-cream-300 mt-1">
              Recent sessions grouped by sessionId and observed funnel steps.
            </p>
              ...
              <ul className="mt-4 space-y-3 max-h-[330px] overflow-auto pr-1">
                {sessionJourneys.slice(0, 12).map((journey) => (
                  ...
                  >
                    ...
                    <p className="text-sm font-medium text-charcoal-600 dark:text-cream-100 mt-1">
                      {journey.sessionId}
                    </p>
                    <p className="text-xs text-charcoal-400 dark:text-cream-300 mt-1">
                      {journey.totalEvents} events · last {formatTime(journey.lastAt)}
                    </p>
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {journey.stepsSeen.length > 0 ? (
                        journey.stepsSeen.map((step) => (
                          <span
                            key={`${journey.sessionId}-${step}`}
                            className="text-[10px] uppercase tracking-wide px-2 py-1 rounded-full bg-cream-100 dark:bg-charcoal-600 text-charcoal-500 dark:text-cream-200"
                          >
                            {step}
                          </span>
                        ))
          ...
          <section className="rounded-2xl bg-white dark:bg-charcoal-500 border border-cream-200 dark:border-charcoal-400 p-5">
            <div className="flex items-center justify-between gap-4">
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-md bg-cream-100 dark:bg-charcoal-600 text-xs text-charcoal-500 dark:text-cream-200"
              >
                Close
              </button>
            </div>
            <p className="mt-2 text-sm text-charcoal-400 dark:text-cream-300">
              {selectedEvent.name} · {formatTime(selectedEvent.at)}
            </p>
